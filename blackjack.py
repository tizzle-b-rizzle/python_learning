...
def player_hit():
    ...
    player_cards = str(int(player_cards + new_player_card))
...
def dealer_hit():
    ...
    dealer_cards = str(int(dealer_cards + new_dealer_card))
# once the dealer has dealt their own card, it will be removed fom the deck
    full_deck.remove(int(new_dealer_card))
    print("The total value of the dealer's cards is " + dealer_cards)
    if int(dealer_cards) > 21:
...
while int(player_cards) < 21 or int(dealer_cards) < 21:
    hit_or_stand_question()

#consider turning player_cards and dealer_cards into empty lists
